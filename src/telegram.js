...
export async function sendMessage(env, chatId, text) {
  ...
  for (const chunk of chunks) {
    await tg(env,"sendMessage",{chat_id:chatId,text:chunk,disable_web_page_preview:true});
  }
}
...
export async function getTelegramFile(env, fileId) {
  ...
  return {bytes: btoa(binary), mime: guessMime(file.file_path)};
}

function guessMime(path="") {
  const ext = path.split(".").pop().toLowerCase();
  return ({pdf:"application/pdf",jpg:"image/jpeg",jpeg:"image/jpeg",png:"image/png",webp:"image/webp",gif:"image/gif"})[ext] || "application/octet-stream";
}
